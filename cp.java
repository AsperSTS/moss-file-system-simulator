...
public class cp
{
  ...
  public static void main( String[] argv ) throws Exception
  {
    ...
    // revisamos si el archivo de inicio existe
    if(Kernel.checkIfDirectory(in_name) == -1){
      System.err.println( PROGRAM_NAME + ":\"" +
          argv[0] + "\" - El archivo o directorio no existe." ) ;
      Kernel.exit(21);
    }
    // revisamos si el archivo de salida existe
    if(Kernel.checkIfDirectory(out_name) == -1){
      System.err.println( PROGRAM_NAME + ":\"" +
          argv[1] + "\" - El archivo o directorio no existe." ) ;
      Kernel.exit(21);
    }
    // revisamos si el archivo de inicio no es un directorio
    if (Kernel.checkIfDirectory(in_name) == 0 ){
      System.err.println( PROGRAM_NAME + ":\"" +
          argv[0] + "\" - Es un directorio no un archivo" ) ;
      Kernel.exit(21);
    }
    // revisamos si el archivo de salida no es un directorio
    if (Kernel.checkIfDirectory(out_name) == 0 ){
      System.err.println( PROGRAM_NAME + ":\"" +
          argv[1] + "\" - Es un directorio no un archivo" ) ;
      Kernel.exit(21);
    }
