...
public class Kernel
{
  ...
  /**
 * Verifica si el archivo o directorio especificado por la ruta es un directorio.
 * @param pathname La ruta del archivo o directorio que se va a verificar.
 * @return 0 si es un directorio, 1 si no es un directorio (archivo regular u otro tipo de archivo) y -1 si el archivo o directorio no existe.
 * @throws Exception Si se produce una excepción durante la verificación.
 */
public static int checkIfDirectory(String pathname) throws Exception {
  // Crea una instancia de IndexNode para almacenar información sobre el archivo o directorio.
  IndexNode indexNode = new IndexNode();
  
  // Obtiene el número de nodo de índice y llena la información del archivo o directorio en el IndexNode.
  int file;
  short indexNodeNumber = findIndexNode(pathname, indexNode);

  if (indexNodeNumber < 0) {
      // Si el número de nodo de índice es negativo, el archivo o directorio no existe.
      // Devuelve -1 para indicar que no existe.
      return -1;
  } else {
      // Comprueba si el archivo o directorio es un directorio al verificar los bits de modo en el IndexNode.
      if ((indexNode.getMode() & S_IFMT) == S_IFDIR) {
          // Si el bit de modo indica que es un directorio, establece 'file' en 0.
          file = 0;
      } else {
          // Si el bit de modo no indica que es un directorio (archivo regular u otro tipo), establece 'file' en 1.
          file = 1;
      }
  }
  return file; // Devuelve 0 si es un directorio, 1 si no es un directorio y -1 si no existe.
}

public static int tryOpenForWriting(String pathname, int flags) {
    try {
        // Intenta abrir el archivo en modo escritura
        int fileDescriptor = open(pathname, flags); // Cambia el tipo de retorno a int

        // Comprueba si la apertura fue exitosa
        if (fileDescriptor >= 0) {
            return 1;
        } else {
            process.errno = -fileDescriptor;  // Establece el código de error adecuadamente
            return -1; // Indicar que no se pudo abrir el archivo
        }
    } catch (Exception e) {
        // Ocurrió un error al abrir el archivo
        // Verifica si es un error relacionado con el sistema de solo lectura (EROFS)
        if (e.getMessage().contains("EROFS")) {
            process.errno = EROFS;  // Establecer el código de error EROFS
        } else {
            // Otro error, puedes manejarlo de acuerdo a tus necesidades
            // Por ejemplo, configurando process.errno con otro código de error
        }
    ...
    throws Exception
  {
    ...
    /*  //Manejamos el error al intentar abrir un archivo de solo lectura en modo escritura
    int inputPath = Kernel.open( pathname , Kernel.O_RDWR ) ;
    if(inputPath == -1) {
        process.errno = EROFS;  // Establecer el código de error EROFS
        System.err.println("Error: El sistema de archivos es de solo lectura (EROFS).");    
        return -1;
    }*/
...
// System.out.println( "dirname = " + dirname.toString() ) ;
      ...
      if( dir < 0 )
      {
        ...
        process.errno = EROFS;  // Establecer el código de error EROFS
        System.err.println("Error: El sistema de archivos es de solo lectura (EROFS).");
        Kernel.exit( EXIT_FAILURE) ; // ??? is this correct YEs
      }
